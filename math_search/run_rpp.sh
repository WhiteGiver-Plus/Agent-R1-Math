...
export BASE_MODEL=''
export PROJECT_NAME='Qwen2.5-7B-it-math_search'
export EXPERIMENT_NAME="reinforce_plus_plus_$(date +%Y%m%d_%H%M%S)"
...
nohup python3 -u -m agent_r1.src.main_agent \
    data.train_files=./data/lean/train.parquet \
    data.val_files=./data/lean/test.parquet \
    ...
    tool.env='math_search' $@ >> "${LOG_FILE}" 2>&1
